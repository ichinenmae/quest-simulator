...
- 複数クエストは可能であれば `---` で区切る
- `---` がない場合でも、`QUEST_TYPE:` の出現を新しいクエストブロックとして扱う
- AIが `## 3=350` のようなMarkdown見出しを付けた場合、アプリ側で `3=350` として正規化する
- 金額は円記号やカンマを除いた数値
- 時刻は `10:00` 形式
...
実際のUberアプリ画面録画を確認して作成した詳細版は [OCR_PROMPT.md](./OCR_PROMPT.md) を使用する。特に、画面上の「+3回」「+500円」を累計件数・累計報酬へ変換すること、拒否回数を配達件数へ混ぜないこと、重複スクリーンショットを除外することを詳細版で規定している。
